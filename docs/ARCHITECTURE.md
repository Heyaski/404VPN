...
- Списание берётся за **привязанные** устройства, а не за время подключения: пока устройство привязано, деньги уходят, даже если VPN выключен. Остановить списание можно только отвязкой устройства — кнопка «Отвязать устройство» на главном экране приложения, `DELETE /api/device`. 0 привязанных устройств → списаний нет.
