...
4. В Xcode выбрать свою Team для таргетов `VPN404` и `VPN404Tunnel`, затем в настройках таргетов убрать `CODE_SIGNING_ALLOWED: NO` (он стоит в `project.yml`, чтобы сборка проходила без учётной записи) и собрать на устройство.
