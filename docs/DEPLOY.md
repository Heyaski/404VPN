...
1. **Дать `core` доступ к сети wg-easy** (разово). Панель слушает только `127.0.0.1` хоста, поэтому из контейнера она недоступна ни по `localhost`, ни через `host.docker.internal` — нужен общий docker-сегмент.

   > **Не подключайте wg-easy к чужой сети** (`docker network connect <сеть> wg-easy`). Второй интерфейс меняет у него маршрут по умолчанию, а правило маскарадинга остаётся привязанным к первому — у VPN-клиентов пропадает интернет. Многодомным должен становиться `core`, а не wg-easy.
   ...
   Затем указать её в `.env` как `WG_NETWORK=<имя>` — compose подключит к ней сервис `core`.

   Если что-то уже подключили к wg-easy по ошибке, откатите и перезапустите его:

   ```bash
   docker network disconnect 404vpn_default wg-easy && docker restart wg-easy
   ```
