...
**Чего в панели пока нет:** рассылок по базе и редактора шаблонов уведомлений. Шаблоны сейчас правятся напрямую в таблице `notification_templates`, рассылок нет вовсе — таблица `broadcasts` создана, но воркер под неё не написан. Это отдельный следующий шаг.
