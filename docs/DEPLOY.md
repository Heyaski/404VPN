...
1. **Подключить wg-easy к сети проекта** (разово). Панель слушает только `127.0.0.1` хоста, поэтому из контейнера она недоступна ни по `localhost`, ни через `host.docker.internal` — контейнеру нужен общий с ней docker-сеть:

   ```bash
   docker network connect 404vpn_default wg-easy
   ```

   Публикация портов и собственная сеть wg-easy при этом не меняются: контейнер просто получает второй интерфейс. Имя сети проверьте через `docker network ls` (обычно `404vpn_default`).
