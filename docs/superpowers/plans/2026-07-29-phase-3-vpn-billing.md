...
- [ ] **Step 1:** написать миграцию, применить `./db/migrate.sh`, проверить `\d devices`.
- [ ] **Step 2:** commit `feat(db): device tokens, wg client binding`.
...
- [ ] **Step 1:** тесты: токен ≥ 43 символов и уникален на 1000 генераций; хэш детерминирован; middleware даёт 401 без заголовка, с мусорным токеном и с отозванным устройством; пропускает валидный.
- [ ] **Step 2:** реализация. **Step 3:** тесты зелёные. **Step 4:** commit.
...
- [ ] **Step 1:** тесты: успешная активация начисляет баланс и отдаёт токен; повторная активация → `already_used`; несуществующий код → `invalid_code`; протухший → `expired`; отозванный → `revoked`; 6-я попытка подряд → 429; регистр и дефисы в коде не важны.
- [ ] **Step 2:** реализация. **Step 3:** тесты. **Step 4:** commit.
...
- [ ] **Step 1:** тесты парсера `.conf` → `TunnelConfig` (включая `Address` с маской, несколько DNS, `AllowedIPs` с IPv6).
- [ ] **Step 2:** `FakeWgProvider` для тестов бизнес-логики (счётчики вызовов, детерминированные ключи).
- [ ] **Step 3:** драйвер wg-easy (сеть). **Step 4:** commit.
...
- [ ] **Step 1:** тесты с `FakeWgProvider`: первый вызов туннеля провижинит и сохраняет `wg_client_id`/`wg_public_key`; повторный не создаёт второго клиента; при `suspended` → 402; сверх лимита → 409; DELETE удаляет клиента и освобождает слот.
- [ ] **Step 2:** реализация. **Step 3:** тесты. **Step 4:** commit.
...
- [ ] **Step 1:** тест: пользователь в `suspended` с балансом 0 → оплата 300 ₽ → статус `active`, `setEnabled(true)` вызван.
- [ ] **Step 2:** реализация. **Step 3:** тесты. **Step 4:** commit.
...
- [ ] **Step 1:** переменные `WG_EASY_URL`, `WG_EASY_PASSWORD`, `WG_ENDPOINT_HOST`; раздел в DEPLOY.md: применить миграцию 002, проверить доступность wg-easy из контейнера core, тест активации кода через `curl`.
- [ ] **Step 2:** commit.
